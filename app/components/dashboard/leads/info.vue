...
<template>
  <article v-if="lead" class="leads-info" :aria-label="t('lead.details')">
    <header class="leads-info__head">
      ...
      <div v-if="lead.signals" class="leads-info__signals">
        <span
          v-if="lead.signals.rating"
          class="leads-info__signal leads-info__signal--rating"
        >
          ★ {{ lead.signals.rating }}
        ...
        <span
          ...
          class="leads-info__signal leads-info__signal--green"
        >
          🚗 {{ t("leads.delivery") }}
        </span>
        <span
          v-if="lead.signals.is24x7"
          class="leads-info__signal leads-info__signal--purple"
        >
          🕐 24/7
        </span>
        <span
          v-if="lead.signals.hasCongestion"
          class="leads-info__signal leads-info__signal--orange"
        >
          📊 {{ t("leads.congestion") }}
        </span>
      </div>
    ...
    >
      <h4 class="leads-info__label">{{ t("leads.contacts") }}</h4>
    ...
    <section v-if="lead.address" class="leads-info__section">
      <div class="leads-info__row">
        <h4 class="leads-info__label">{{ t("leads.location") }}</h4>
    ...
    <section class="leads-info__section">
      <h4 class="leads-info__label">{{ t("leads.fitsFor") }}</h4>
    ...
    <section v-if="lead.reasons?.length" class="leads-info__section">
      <h4 class="leads-info__label">{{ t("leads.whyHighLead") }}</h4>
    ...
    <section class="leads-info__section">
      <h4 class="leads-info__label">{{ t("leads.freshness") }}</h4>
        ...
        >
          <div class="leads-info__branch-header">
            <span class="leads-info__branch-name">{{
              branch.name || t("leads.unnamed")
            }}</span>
            <div class="leads-info__branch-badges">
              <span
                v-if="branch.congestion?.present"
                class="leads-info__branch-cong"
                :title="t('leads.hasCongestionData')"
                >📊</span
              >
              <span
                :class="[
              ...
              >
                {{ branch.isActive ? t("leads.active") : t("leads.inactive") }}
              </span>
            </div>
          </div>
  ...
  <article
    v-else
    class="leads-info leads-info--empty"
    :aria-label="t('lead.details')"
  >
    {{ t("leads.selectLeadForDetails") }}
  </article>
</template>
...
.leads-info__signal--rating {
  background: #fff8e1;
  color: #e65100;
}
.leads-info__signal--green {
  background: #e8f5e9;
  color: #298267;
}
.leads-info__signal--purple {
  background: #ede7f6;
  color: #5e35b1;
}
.leads-info__signal--orange {
  background: #fff3e0;
  color: #e65100;
}
.leads-info__signal-sub {
  font-weight: 400;
  opacity: 0.8;
}
...
.leads-info__branch-badges {
  display: flex;
  align-items: center;
  gap: 4px;
  flex-shrink: 0;
}
.leads-info__branch-cong {
  font-size: 9px;
}
