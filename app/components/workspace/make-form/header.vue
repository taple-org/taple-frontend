...
<template>
    <header class="header">
        <div class="header__actions">
            ...
            <Icon name="my-icon:close" class="close" height="12" width="12" @click="controller.close" />
        </div>

        <section class="progress-bar">
            <span class="progress-bar__label">{{ percent }}% Завершено</span>
            <Steps.List class="progress-bar__list">
                <Steps.Item v-for="index in props.steps" :key="index" :index="index - 1" class="progress-bar__item">
                    <Steps.Indicator class="progress-bar__indicator" />
                </Steps.Item>
            </Steps.List>
            <span class="progress-bar__steps">Шаг {{ props.current }} из {{ props.steps }}</span>
        </section>

        <div class="header__title" v-if="title">{{ props.title }}</div>
        <div class="header__description" v-if="props.description">{{ props.description }}</div>
    </header>
</template>
<style scoped>
.header__actions {
