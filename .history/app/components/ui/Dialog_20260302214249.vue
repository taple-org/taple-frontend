...
.dialog-backdrop {
  ...
  /* blur вынесен отдельно чтобы анимировать только opacity */
...
.dialog-content {
  ...
  overflow: hidden;
  pointer-events: all;
  /* GPU-слой создаётся заранее, нет скачка при старте анимации */
...
.dialog-close:hover {
  background-color: color-mix(in srgb, var(--color-neutral-lm) 80%, black);
}
